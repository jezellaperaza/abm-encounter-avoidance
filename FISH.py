...
# def fish_within(point, turbine):
#     x, y = point
#     turbine_x, turbine_y = turbine.position
#     radius = turbine.radius
#
#     distance = np.sqrt((x - turbine_x) ** 2 + (y - turbine_y) ** 2)
#
...
def desired_new_heading(fish: Fish, world: World):
    ...
    for other, distance in others:
        ...
        attraction_orientation_direction = (informed_direction + social_direction) + (strength * fish.AVOIDANCE_DIRECTION)
...
def main():
    ...
    def animate(_):
        ...
        #     if fish_within(f.position, [world.turbines[0].position]):
